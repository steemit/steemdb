...
func syncChunk(ctx context.Context, api *sdkv2.API, mongoClient *mongo.Client, from, to int64) (int64, error) {
	...
	for _, wb := range blocks {
		...
		modelOps = append(modelOps, convertBlockOps(uint32(wb.BlockNum), opsAll[wb.BlockNum], opsVirt[wb.BlockNum])...)
	}
...
func followBlock(ctx context.Context, api *sdkv2.API, mongoClient *mongo.Client, blockNum int64) (int64, error) {
	...
	modelOps := convertBlockOps(uint32(blockNum), opsAll, opsVirt)
...
func convertBlockOps(blockNum uint32, all, virtual []*protocol.OperationObject) []*model.Operation {
	...
	appendOp := func(opObj *protocol.OperationObject, what string) {
		...
		if err != nil {
			...
			log.Printf("[LiveSync] Drop %s op in block %d (trx=%d op=%d): %v",
